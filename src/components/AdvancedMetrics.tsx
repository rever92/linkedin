...
const AdvancedMetrics = ({ data, filteredData }) => {
  const [timeAnalysis, setTimeAnalysis] = useState([]);
  const [lengthAnalysis, setLengthAnalysis] = useState([]);
  ...
  const analyzeData = () => {
    try {
      ...
      const timeAggregated = _.groupBy(timeStats, item => `${item.dayOfWeek}-${item.hour}`);
      const timeAnalysisData = Object.entries(timeAggregated)
        .map(([key, posts]) => {
          const [day, hour] = key.split('-').map(Number);
          const avgEngagement = _.meanBy(posts, 'engagement');
          return { 
            day, 
            hour, 
      ...
      const lengthData = data
        .filter(post => post.text && post.views > 0)
        .map(post => ({
      ...
      const keywordsData = Object.entries(wordStats)
        .map(([word, count]) => ({ 
          word, 
          count,
  ...
  const prepareHeatmapData = () => {
    ...
    // Filtrar celdas con menos de 3 posts y ajustar escala
    const validCells = heatmapData.flat().filter(cell => cell.posts >= 3);
    if (validCells.length === 0) return heatmapData;

    const engagements = validCells.map(cell => cell.engagement);
    ...
    return heatmapData.map(row =>
      row.map(cell => ({
        ...cell,
        // Si tiene menos de 3 posts, engagement = 0 para que no se muestre
        normalizedEngagement: cell.posts >= 3 
          ? (cell.engagement - minEngagement) / range
          : 0
      }))
    );
  };
  ...
  const getColorStyle = (cell) => {
    if (cell.posts < 3) return { backgroundColor: '#f9fafb' }; // gray-50
    ...
    <div className="grid gap-4 grid-cols-1 md:grid-cols-2">
      {/* {times && (
        <Card className="col-span-full">
          <CardHeader>
            <CardTitle>Mejores y Peores Momentos para Publicar</CardTitle>
          </CardHeader>
          <CardContent>
            <div className="grid gap-4 grid-cols-1 md:grid-cols-2">
              <div className="space-y-4">
                <h3 className="font-semibold text-green-700">Top 3 Mejores momentos</h3>
                {times.best.map((time, index) => (
                  <div key={index} className="p-3 bg-green-50 rounded-lg">
                    <p className="text-green-600">
                      {time.day} a las {time.hour}:00
                      <br />
                      Engagement promedio: {time.engagement}%
                      <br />
                      <span className="text-sm">Basado en {time.posts} posts</span>
                    </p>
                  </div>
                ))}
              </div>
              <div className="space-y-4">
                <h3 className="font-semibold text-red-700">3 Peores momentos</h3>
                {times.worst.map((time, index) => (
      ...
      {lengthAnalysis.length > 0 && (
        <Card className="lg:col-span-1">
          <CardHeader>
            <CardTitle>Correlación: Longitud vs Engagement</CardTitle>
          </CardHeader>
      ...
      <Card className="col-span-full">
        <CardHeader>
          <CardTitle>Mapa de Calor: Engagement por Día y Hora</CardTitle>
        </CardHeader>
