...
export default function PlannerAnalyticsView({ taxonomies }: Props) {
  ...
  const cards = summary ? [
    ...
    { label: 'Interacciones', value: number.format(summary.interactions), detail: `${number.format(summary.likes)} reacciones`, icon: Heart, color: 'text-rose-700 bg-rose-50' },
    { label: 'Engagement', value: `${summary.engagement_rate.toLocaleString('es-ES')}%`, detail: 'sobre visualizaciones', icon: Gauge, color: 'text-violet-700 bg-violet-50' },
  ] : [];
