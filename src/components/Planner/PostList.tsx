...
import { Post, PostState, getPostId, getPostMetric } from '../../types/posts';
  ...
  return <section className="space-y-5">
    ...
    <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-3">
      {filtered.map(post => <button key={getPostId(post)} onClick={() => onPostSelect(post)} className="group rounded-2xl border border-slate-200 bg-white p-5 text-left shadow-sm transition hover:-translate-y-0.5 hover:border-blue-300 hover:shadow-md">
        ...
        {post.state === 'publicado' && <div className="mt-4 grid grid-cols-4 gap-1 rounded-xl border border-emerald-100 bg-emerald-50 px-2 py-2 text-[11px] font-medium text-emerald-800"><span className="flex items-center justify-center gap-1" title="Impresiones"><Eye className="h-3.5 w-3.5" />{getPostMetric(post, 'impresiones').toLocaleString('es-ES')}</span><span className="flex items-center justify-center gap-1" title="Reacciones"><Heart className="h-3.5 w-3.5" />{getPostMetric(post, 'reacciones').toLocaleString('es-ES')}</span><span className="flex items-center justify-center gap-1" title="Comentarios"><MessageCircle className="h-3.5 w-3.5" />{getPostMetric(post, 'comentarios').toLocaleString('es-ES')}</span><span className="flex items-center justify-center gap-1" title="Compartidos"><Repeat2 className="h-3.5 w-3.5" />{getPostMetric(post, 'compartidos').toLocaleString('es-ES')}</span></div>}
